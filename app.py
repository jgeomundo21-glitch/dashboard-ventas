...
def load_data():
    ...
    # Crear columna de semana para el análisis semanal (Ej: 2026-W26)
    df["Semana"] = (
        df["Fecha y hora de la encuesta"]
        .dt.to_period("W")
        .astype(str)
...
# --- TÍTULO Y LEYENDA LEGAL EN LA CABECERA ---
st.title("📈 Dashboard Interactivo de Ventas Mystic")
st.markdown(
    """
...
# Aplicar Filtros Globales (General)
df_filtered = df.copy()
...
with row2_col2:
    ...
    st.subheader(f"📅 Tendencia {tipo_tendencia} ({metrica_seleccionada})")
    
    if not df_filtered.empty:
        columna_tiempo = "Fecha" if tipo_tendencia == "Diaria" else "Semana"
        
        df_time = (
            df_filtered.groupby(columna_tiempo)[columna_metrica].sum().reset_index()
        )
        fig_time = px.line(
            df_time,
            x=columna_tiempo,
        ...
        fig_time.update_layout(xaxis_title=tipo_tendencia, yaxis_title=etiqueta_eje)
...
with row3_col2:
    st.subheader(
        f"🎯 Comisión Acumulada (Venta Directa > $120) - por Empleado"
        f" ({metrica_seleccionada})"
    )

    df_comision = df.copy()
    if selected_empleado != "Todos":
        df_comision = df_comision[df_comision["Empleado"] == selected_empleado]
    if selected_regional != "Todas":
        df_comision = df_comision[df_comision["Regional"] == selected_regional]
    if selected_supercat != "Todas":
        df_comision = df_comision[
            df_comision["Supercategoría"] == selected_supercat
        ]
    if selected_modelo != "Todos":
        df_comision = df_comision[
            df_comision["Informar modelo"] == selected_modelo
        ]
    if selected_pdv != "Todos":
        df_comision = df_comision[df_comision["PDV"] == selected_pdv]

    df_comision = df_comision[
        df_comision["Venta Directa o Indirecta"]
        .str.contains("directa", case=False, na=False)
        & (df_comision["Total Venta"] >= 120)
    ]
...
# --- SECCIÓN DE SOPORTE: BASE DE DATOS COMPLETA FILTRADA ---
st.subheader("📋 Base de Datos de Soporte (Registros Filtrados)")
st.markdown(
    "Esta tabla contiene **todas las columnas originales** de la base de datos"
    " aplicando los filtros seleccionados arriba:"
)
...
st.download_button(
    ...
    data=csv_data,
    file_name="base_datos_soporte_filtrada.csv",
    mime="text/csv",
)